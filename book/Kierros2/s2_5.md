...
def ratkaise_lampotila(p, V, n):
    return p * V / (n * R)
```
<p> Luodaan moduulin *ideaalikaasu.py* kanssa samaan hakemistoon tiedosto *testi.py*, jossa hyödynnämme *ideaalikaasu*-moduulia
**import**-avainsanan avulla: </p>
...
```
<p>Toinen tapa on tuoda *ideaalikaasu*-moduulista vain tietyt funktiot ja muuttujat *testi.py*-ohjelman käyttöön. Tähän
käytetään käskyä **from** MODUULI **import** FUNKTIOT </p>
